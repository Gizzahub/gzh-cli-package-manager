# .make/docker.mk - Docker targets for package manager testing
# Container build, optimization, and testing for multi-distro package managers
...
.PHONY: docker-test-ubuntu docker-test-arch docker-test-alpine
...
# ==============================================================================
# Package Manager Testing (multi-distro)
# ==============================================================================

docker-test-ubuntu: ## Test in Ubuntu container (apt, snap)
	@echo "Testing in Ubuntu environment..."
	@docker run --rm -v $(PWD):/workspace -w /workspace ubuntu:latest bash -c "\
		apt-get update && apt-get install -y wget && \
		./build/$(BINARY_NAME) detect"

docker-test-arch: ## Test in Arch Linux container (pacman, yay)
	@echo "Testing in Arch Linux environment..."
	@docker run --rm -v $(PWD):/workspace -w /workspace archlinux:latest bash -c "\
		pacman -Sy --noconfirm wget && \
		./build/$(BINARY_NAME) detect"

docker-test-alpine: ## Test in Alpine Linux container (apk)
	@echo "Testing in Alpine Linux environment..."
