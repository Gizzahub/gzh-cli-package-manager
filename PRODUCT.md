...
- 소비자 최소 버전은 Go 1.24.0+ (`go.mod`)이며 로컬 개발과 정규 CI는 최신
  1.26 패치인 Go 1.26.7 (`toolchain go1.26.7`)를 권장·고정한다. Go 1.24.11
  호환성 CI를 별도로 유지하며 ADR-004가 지원 계약이다. CGO 미사용 (순수 정적 빌드)
