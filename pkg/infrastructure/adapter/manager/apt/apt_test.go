	...
	tests := []struct {
	...
	}{
		{
			...
			execFunc: func(_ context.Context, command string, args ...string) (*output.ExecutionResult, error) {
				if command == "which" && len(args) == 1 && args[0] == "apt" {
					return &output.ExecutionResult{
						Stdout:   "/usr/bin/apt\n",
						ExitCode: 0,
					}, nil
...
func TestAdapter_GetVersion(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			execFunc: func(_ context.Context, command string, args ...string) (*output.ExecutionResult, error) {
				if command == "apt" && len(args) == 1 && args[0] == "--version" {
					return &output.ExecutionResult{
...
func TestAdapter_ListPackages(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			execFunc: func(_ context.Context, command string, args ...string) (*output.ExecutionResult, error) {
				// apt list --installed
				if command == "apt" && len(args) == 2 && args[0] == "list" && args[1] == "--installed" {
					return &output.ExecutionResult{
						Stdout: `Listing...
...
`,
				...
				// apt list --upgradable
				if command == "apt" && len(args) == 2 && args[0] == "list" && args[1] == "--upgradable" {
					return &output.ExecutionResult{
						Stdout: `Listing...
...
`,
		...
		},
		{
			...
			execFunc: func(_ context.Context, command string, args ...string) (*output.ExecutionResult, error) {
				if command == "apt" && args[0] == "list" && args[1] == "--installed" {
					return &output.ExecutionResult{
						Stdout: `Listing...
vim/jammy-updates,now 2:8.2.3995-1ubuntu2.12 amd64 [installed]
`,
				...
				if command == "apt" && args[0] == "list" && args[1] == "--upgradable" {
					return &output.ExecutionResult{
						Stdout:   "Listing...\n",
						ExitCode: 0,
					}, nil
...
func TestAdapter_CheckHealth(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "healthy system",
			execFunc: func(_ context.Context, command string, args ...string) (*output.ExecutionResult, error) {
				if command == "apt-get" && len(args) == 1 && args[0] == "check" {
					return &output.ExecutionResult{
		...
		},
		{
			...
			execFunc: func(_ context.Context, command string, args ...string) (*output.ExecutionResult, error) {
				if command == "apt-get" && args[0] == "check" {
					return &output.ExecutionResult{
						Stdout:   "Reading package lists... Done\n",
						ExitCode: 0,
					}, nil
